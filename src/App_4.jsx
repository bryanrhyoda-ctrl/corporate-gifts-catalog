...
export default function App() {
  ...
  const editLeadTime = (id) => {
    ...
    setNewLeadTimeId(lt.id);
    setNewLeadTimeLabel(lt.label);
    setNewLeadTimeSub(lt.sub);
  };

  // Lead Times: Save Edit
  const saveLeadTimeEdit = () => {
    if (!newLeadTimeLabel.trim()) {
      alert("Fill label");
      return;
    }
    setLeadTimes(leadTimes.map(lt => 
      lt.id === editingLeadTimeId 
        ? { id: lt.id, label: newLeadTimeLabel, sub: newLeadTimeSub }
        : lt
    ));
  ...
  return (
    <div style={{ fontFamily: "'Inter', system-ui, sans-serif", background: "#f8f8f6", minHeight: "100vh", padding: "0 0 60px" }}>
      ...
      {showLeadTimes && isAdmin && (
        <div style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, background: "rgba(0,0,0,0.5)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000 }}>
          <div style={{ background: "#fff", borderRadius: 12, padding: 32, maxWidth: 600, width: "90%", maxHeight: "85vh", overflowY: "auto", boxShadow: "0 10px 40px rgba(0,0,0,0.2)" }}>
            <h2 style={{ fontSize: 18, fontWeight: 700, color: "#1a1a1a", marginBottom: 20 }}>⏱️ Manage Lead Times</h2>
            
            <div style={{ marginBottom: 24, padding: 16, background: "#f5f5f5", borderRadius: 8 }}>
              <h3 style={{ fontSize: 13, fontWeight: 600, color: "#1a1a1a", marginBottom: 12 }}>{editingLeadTimeId ? "Edit" : "Add New"}</h3>
              <input type="text" value={newLeadTimeId} onChange={e => setNewLeadTimeId(e.target.value)} placeholder="ID (e.g., L5)" disabled={editingLeadTimeId} style={{ width: "100%", boxSizing: "border-box", padding: "10px", fontSize: 13, border: "1px solid #ddd", borderRadius: 6, marginBottom: 8, fontFamily: "inherit", background: editingLeadTimeId ? "#eee" : "#fff" }} />
              <input type="text" value={newLeadTimeLabel} onChange={e => setNewLeadTimeLabel(e.target.value)} placeholder="Label (e.g., Express)" style={{ width: "100%", boxSizing: "border-box", padding: "10px", fontSize: 13, border: "1px solid #ddd", borderRadius: 6, marginBottom: 8, fontFamily: "inherit" }} />
              <input type="text" value={newLeadTimeSub} onChange={e => setNewLeadTimeSub(e.target.value)} placeholder="Time (e.g., 1–2 days)" style={{ width: "100%", boxSizing: "border-box", padding: "10px", fontSize: 13, border: "1px solid #ddd", borderRadius: 6, marginBottom: 8, fontFamily: "inherit" }} />
              <div style={{ display: "flex", gap: 8 }}>
                <button onClick={editingLeadTimeId ? saveLeadTimeEdit : addLeadTime} style={{ flex: 1, padding: "10px", background: "#c8a96e", color: "#fff", border: "none", borderRadius: 6, fontSize: 13, fontWeight: 600, cursor: "pointer" }}>{editingLeadTimeId ? "Update" : "Add"}</button>
                {editingLeadTimeId && <button onClick={() => { setEditingLeadTimeId(null); setNewLeadTimeId(""); setNewLeadTimeLabel(""); setNewLeadTimeSub(""); }} style={{ flex: 1, padding: "10px", background: "#888", color: "#fff", border: "none", borderRadius: 6, fontSize: 13, fontWeight: 600, cursor: "pointer" }}>Cancel</button>}
              </div>
            </div>
            ...
            <div>
              ...
              {leadTimes.map(lt => (
                <div key={lt.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: 12, background: "#f5f5f5", borderRadius: 6, marginBottom: 8 }}>
                  <div><strong>{lt.label}</strong> ({lt.sub})</div>
                  <div style={{ display: "flex", gap: 6 }}>
                    <button onClick={() => editLeadTime(lt.id)} style={{ padding: "5px 10px", background: "#666", color: "#fff", border: "none", borderRadius: 4, fontSize: 11, fontWeight: 600, cursor: "pointer" }}>Edit</button>
                    <button onClick={() => deleteLeadTime(lt.id)} style={{ padding: "5px 10px", background: "#ff5555", color: "#fff", border: "none", borderRadius: 4, fontSize: 11, fontWeight: 600, cursor: "pointer" }}>Delete</button>
                  </div>
                </div>
              ))}
            </div>
      ...
      {showCategories && isAdmin && (
        <div style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, background: "rgba(0,0,0,0.5)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000 }}>
          <div style={{ background: "#fff", borderRadius: 12, padding: 32, maxWidth: 600, width: "90%", maxHeight: "85vh", overflowY: "auto", boxShadow: "0 10px 40px rgba(0,0,0,0.2)" }}>
            <h2 style={{ fontSize: 18, fontWeight: 700, color: "#1a1a1a", marginBottom: 20 }}>📂 Manage Categories</h2>
            
            <div style={{ marginBottom: 24, padding: 16, background: "#f5f5f5", borderRadius: 8 }}>
              <h3 style={{ fontSize: 13, fontWeight: 600, color: "#1a1a1a", marginBottom: 12 }}>{editingCategoryName ? "Edit" : "Add New"}</h3>
              ...
              <div style={{ display: "flex", gap: 8 }}>
                <button onClick={editingCategoryName ? saveCategoryEdit : addCategory} style={{ flex: 1, padding: "10px", background: "#c8a96e", color: "#fff", border: "none", borderRadius: 6, fontSize: 13, fontWeight: 600, cursor: "pointer" }}>{editingCategoryName ? "Update" : "Add"}</button>
                {editingCategoryName && <button onClick={() => { setEditingCategoryName(null); setNewCategoryName(""); }} style={{ flex: 1, padding: "10px", background: "#888", color: "#fff", border: "none", borderRadius: 6, fontSize: 13, fontWeight: 600, cursor: "pointer" }}>Cancel</button>}
              </div>
            </div>
            ...
            <div>
              <h3 style={{ fontSize: 13, fontWeight: 600, color: "#1a1a1a", marginBottom: 10 }}>Current</h3>
              {categories.map(cat => (
                <div key={cat} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: 12, background: "#f5f5f5", borderRadius: 6, marginBottom: 8 }}>
                  <div><strong>{cat}</strong></div>
                  <div style={{ display: "flex", gap: 6 }}>
                    <button onClick={() => editCategory(cat)} style={{ padding: "5px 10px", background: "#666", color: "#fff", border: "none", borderRadius: 4, fontSize: 11, fontWeight: 600, cursor: "pointer" }}>Edit</button>
                    <button onClick={() => deleteCategory(cat)} style={{ padding: "5px 10px", background: "#ff5555", color: "#fff", border: "none", borderRadius: 4, fontSize: 11, fontWeight: 600, cursor: "pointer" }}>Delete</button>
                  </div>
                </div>
              ))}
            </div>
